...
bool sorting(const Card &a, const Card &b) {return a.getLifeTime() < b.getLifeTime(); }
...
int main(int argc, char*args[])
{
    ...
    if (!initWindow()) {
        ...
        /*for (int i = 0; i < theCards.size(); i++) {
            theTextures[i] = loadingT(theCards[i].getPath());
            theCards[i].texIndex = i;
        }*/
        ...
        while (!quit) {
            ...
            std::sort(theCards.begin(), theCards.end(), sorting);
            
            for (int i = 0; i < theCards.size(); i++) {
                if (theCards[i].getLifeTime() < clock()) {
                    theCards[i].move();
                }
                theCards[i].render(gRenderer); // theTextures[theCards[i].texIndex]
            }
...
bool readXml(std::string filePath, std::vector<Card> &vecCard) {
    ...
    for (rapidxml::xml_node<>* second = content->first_node("media"); second; second = second->next_sibling()) {
        ...
        vecCard.push_back(Card(cardCat, seHeader, seText, enHeader, enText, position, velocity, mediaPath));
